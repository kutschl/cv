...
def optimize_snake_triple_dp(snake_pts, grad_img, grad_norm,
                             window_radius=3,
                             alpha=0.05,
                             beta=0.1,
                             gamma=2.0):
    """
    DP mit Dreier-Potential:
      - Zustand s_i = (k_{i-1}, k_i)
      - Energie über (v_{i-2}, v_{i-1}, v_i)

    snake_pts: (N,2) int/float, [x,y] oder [col,row] – hier: [x,y]
    grad_img: nur für shape gebraucht
    grad_norm: |∇I|^2 normiert 0..1 (gleiche Größe wie grad_img)
    """

    N = len(snake_pts)

    # 1. Fenster und Unary-Werte vorbereiten
    windows = []    # Kandidatenkoordinaten (int) pro Punkt
    windows_f = []  # float-Koordinaten für Distanzberechnungen
    unary   = []    # U_i(k) = -gamma * grad_norm[y,x], shape (M_i,)
    ...
    # mittlere Segmentlänge d aus der initialen Snake
    d = compute_mean_dist(snake_pts)

    # 2. DP initialisieren für i = 1 (erste "echte" Paar-Stufe)
    K0 = windows_f[0]
    K1 = windows_f[1]
    M0, M1 = len(K0), len(K1)
    ...
    # Backpointer-Liste für i >= 2
    backptr = []   # jedes Element: Array (M_{i-1}, M_i) mit Index l* (bestes k_{i-2})
    ...
    for i in range(2, N):
        ...
        for j in range(M_im1):        # Kandidat für v_{i-1}
            v_im1 = K_im1[j]
            prev_cost = C_prev[:, j]                      # (M_im2,)

            diff_vi = K_i - v_im1                         # (M_i, 2)
            dist = np.linalg.norm(diff_vi, axis=1)
            unary_term = unary[i] + alpha * (dist - d) ** 2  # (M_i,)

            base_curv = K_i - 2.0 * v_im1                 # (M_i, 2)
            curv = base_curv[:, None, :] + K_im2[None, :, :]
            curv_cost = beta * np.sum(curv * curv, axis=2)    # (M_i, M_im2)
        ...
        backptr.append(BP_i)
        C_prev = C_curr   # für nächste Stufe

    # 4. Backtracking

    # letzter Zustand: (j*, k*) mit minimaler Kosten
    last_j, last_k = np.unravel_index(np.argmin(C_prev), C_prev.shape)

    idx = [None] * N    # hier landen die gewählten Kandidaten-Indizes pro Punkt
    idx[N-1] = last_k
    idx[N-2] = last_j

    # rückwärts über i = N-1..2
    for t in range(len(backptr)-1, -1, -1):
        i = t + 2       # dieses BP gehört zu Schritt i
        BP_i = backptr[t]
        l = BP_i[idx[i-1], idx[i]]
        idx[i-3+1] = l   # entspricht Index für v_{i-2}
        # Verschiebung:
        # i    -> v_i
        # i-1  -> v_{i-1}
        # i-2  -> v_{i-2}
        # damit landen wir schrittweise bei allen v_0..v_{N-1}

    # 5. Kandidaten-Koordinaten einsammeln
    new_snake = []
    for i in range(N):
        K_i = windows[i]
        k_i = idx[i]
...
# gradient image
grad_img, grad_norm = compute_gradient(uav_img)
if False:
    viz_image((grad_img*255).astype(np.uint8))
...
mask_cont = cv2.Canny(mask_cont, 100, 200)
